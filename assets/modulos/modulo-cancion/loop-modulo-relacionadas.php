<?php
if (!isset($album_id_loop)) return;

// Asegurar que $artista_nombre esté definida
if (!isset($artista_nombre)) {
    $artista_nombre = 'Artista';
}

// Obtener la portada del álbum para usarla como fallback si la canción no tiene imagen
...
if ($query->have_posts()) :
    while ($query->have_posts()) : $query->the_post();
        ...
        // Usar la imagen de la canción, o la del álbum como fallback
...
?>
        <div class="bd-song-card d-flex align-items-center gap-3 p-2 rounded-3">
            <div class="bd-song-thumb-wrap position-relative flex-shrink-0" style="width:52px;height:52px;">
                <img src="<?php echo esc_url($thumb_url); ?>" alt="<?php echo esc_attr(get_the_title()); ?>" class="w-100 h-100 rounded-2 object-fit-cover">
